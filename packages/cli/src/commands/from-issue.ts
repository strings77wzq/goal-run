...
export async function fromIssueCommand(
  ...
  if (issueInfo) {
    ...
    console.log(pc.dim(`Parsed GitHub issue: ${issueInfo.owner}/${issueInfo.repo}#${issueInfo.issueNumber}`));
  } else {
  ...
  const outputPath = opts.output ?? `.goalrun/goals/${input.replace(/[^a-z0-9]+/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "").slice(0, 40)}.yaml`;
