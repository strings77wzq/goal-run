...
export async function runCommand(
  ...
  if (opts.dryRun) {
    ...
    if (opts.loop) {
      ...
      console.log(pc.dim(`  goalrun resume ${timestamp}`));
  ...
  if (opts.json) {
    console.log(JSON.stringify({ run_dir: runDir, plan: planReport, run_state: runState }, null, 2));
  }
}
