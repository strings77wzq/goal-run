...
export function checkCompleteness(criteria: string[], filePath: string): Diagnostic[] {
  ...
  if (!hasErrorPath) {
    diagnostics.push(
      createInfo('CRITERIA_MISSING_ERROR_PATH', 'No criteria cover error or edge case handling', {
        file: filePath,
  ...
  if (!hasRegression) {
    diagnostics.push(
      createInfo('CRITERIA_MISSING_REGRESSION', 'No criteria cover regression prevention', {
        file: filePath,
