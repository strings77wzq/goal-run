...
import { DiagnosticSchema, createError, createWarning, createInfo, type Diagnostic } from "../src/diagnostic.js";
...
describe("DiagnosticSchema", () => {
  ...
  it("rejects zero or negative line numbers", () => {
    expect(DiagnosticSchema.safeParse({ code: "E", severity: "error", message: "x", line: 0 }).success).toBe(false);
    expect(DiagnosticSchema.safeParse({ code: "E", severity: "error", message: "x", line: -1 }).success).toBe(false);
  });

  it("rejects non-integer line numbers", () => {
    const result = DiagnosticSchema.safeParse({ code: "E", severity: "error", message: "x", line: 3.5 });
    expect(result.success).toBe(false);
  });
...
describe("createError", () => {
  ...
  it("creates an error diagnostic with optional fields", () => {
    const d = createError("E002", "bad config", { file: "config.yaml", line: 10, hint: "check schema" });
