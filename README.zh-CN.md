...
```bash
goalrun run .goalrun/goals/fix-login-timeout.yaml --loop --isolated
```
...
```bash
# 一键推进，自动跳过中间状态，在人类决策点停下
goalrun advance <run-id>
```

`advance` 会在这些地方自动停下等你：

- `waiting_for_user` — Agent 产出后需要你审核
- `blocked_by_policy` — 违反安全策略，需要你决策
...
**`advance` 自动推进所有非决策状态，只在需要人类判断的地方停下。** `resume --to` 让你手动精确控制每一个状态。
...
| `goalrun run <goal> --loop --isolated`                            | 创建可检查点的受监督运行（带 worktree 隔离） |
| `goalrun advance <run-id>`                                        | 半自动推进，只在人类决策点停下               |
| `goalrun resume <run-id>`                                         | 手动推进到指定状态                           |
...
- **不执行命令**：验证命令只检查不执行
- **结构化 git 参数**：所有 git 操作使用参数数组（不使用 shell 字符串拼接）
- **Worktree 隔离**：`--isolated` 创建独立 git worktree；rollback 只删除 GoalRun 管理的分支
...
| P3 — Git worktree 隔离、diff 捕获、回滚                 | ✅   |
| P4 — Adapter 层（Claude/Codex/Cursor/OpenCode handoff） | ✅   |
